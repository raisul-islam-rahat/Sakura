...
## Active romance artwork — September 12, 2026

The active assets are `hand-kiss-approved-alpha-8.webp`, `dance-spin-alpha-8.webp`, and `dance-lean-alpha-8.webp`. Each is a lossless 2560×1280 WebP atlas with eight padded 640×640 cells and real alpha transparency. Atlas dimensions include padding and normalization; the image generator returned 1774×887 for kiss/lean and 1536×1024 for spin, so these are not native 4K illustrations.

The approved hand-kiss artwork is retained. Its alpha is preserved; extraction removes only detached neighboring-cell fragments. Spin frame 0 equals kiss frame 7, and lean frame 0 equals spin frame 7. The dance plays both sheets once in order, two poses per second, eight seconds total. `config.js` controls `danceFPS` and `danceCycles`. No frame blending is used, to avoid doubled faces or limbs.

Generated using built-in imagegen. Prompt set: (kiss) eight adjacent hand-lift, gentle lip-to-hand contact, linger, release and rise poses; (spin) same adult pink-gown/white-suit couple, eight consecutive underarm-turn stages from handhold through front/side/back/return into ballroom hold; (lean) eight consecutive supported backward-lean and recovery poses. All prompts requested high native detail, consistent anatomy, full silhouettes, fixed camera/scale, transparent alpha and no green backdrop. The dance generator returned painted neutral checkerboards; connected-background extraction converts these into actual alpha while preserving enclosed costume whites. No green matte was generated or keyed in this replacement.

Source identifiers are in `tools/transparent-romance-sources.json`; build with `node tools/build-transparent-romance.cjs`. The earlier `build-continuous-romance.cjs` is obsolete and must not be used for the current assets. `tests/verify-continuous-romance.cjs` checks actual transparency, frame bounds, distinct poses and both matching seams. `tests/verify-romance.cjs` verifies timing, phase order, pause, camera reset and cake continuation. The browser celebration test verifies the hand-kiss rendering and cake-to-sky flow. Preview: `output/continuous-romance-playback.webp`.

Manual dance-sheet cleanup: unified-spin.webp and unified-lean.webp are 1672 x 941 PNG-encoded files with alpha despite their extensions. Browsers decode them correctly. actor.json now supplies individual frame reference heights and anchors matched to the prior poses, including the lean row boundary at y=508. Timing is unchanged. Source images were not resized or modified. Registration report: output/manual-dance-registration.json; transition preview: output/manual-dance-seams.png. Earlier actor metadata is backed up in backup/actor-before-manual-dance-resize.json. Historical builders should not overwrite these manual edits.

Manual intro update: unified-intro.webp is a 1672 x 941 PNG-encoded image with alpha. intro-giving now uses individual frame crops, scales and anchors matched to the previous introduction. Artwork and handover timing are unchanged. Previous metadata: backup/actor-before-manual-intro-resize.json. Registration: tools/register-manual-intro.cjs; report: output/manual-intro-registration.json.
