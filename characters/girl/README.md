...
Transparent PNG update: walk-padded-alpha.png, bouquet-walk-fixed-alpha.png, and bouquet-pickup-standing-fixed-alpha.png now store real alpha. actor.json uses these files with no runtime green chroma key. Exact existing frame placement and normalized walk heights are preserved. Built-in imagegen background-extraction prompt requested the same eight poses, exact positions, clean edges and actual transparency; its output contained a painted checkerboard and was rejected. Final assets were exported from the existing clean canvas renderer using tools/bake-girl-alpha.cjs. Verified all three PNG alpha channels and transparent background pixels.

Walking dress colour match: walk-colour-matched.webp and bouquet-walk-colour-matched.webp match the fabric palette of standing frame 0 in bouquet-pickup-standing-fixed-alpha.png. A soft pink-fabric mask adjusts only colour while preserving the original dimensions and alpha byte-for-byte. Frame positions, details and cycle timing are unchanged. Build: tools/match-walking-dress.cjs. Built-in imagegen was also tried with the prompt 'colour-only edit, match standing dress and heels, preserve exact poses/grid/skin/hair/bouquet/alpha'; it returned a checkerboard background, so that candidate was not shipped. The final calibrated colour export preserves original cutouts. verify-girl-edges.cjs and verify-bouquet.cjs passed, and both exported alpha channels were checked against the originals.
