...
function neutralAlpha(d,w,h,tolerance=5){const seen=new Uint8Array(w*h),queue=new Int32Array(w*h);for(let p=0;p<w*h;p++){if(seen[p])continue;const eligible=p=>{const i=p*4;return Math.max(d[i],d[i+1],d[i+2])-Math.min(d[i],d[i+1],d[i+2])<=tolerance&&d[i]>100;};if(!eligible(p)){seen[p]=1;continue;}let head=0,end=1;queue[0]=p;seen[p]=1;let border=false;while(head<end){const q=queue[head++],x=q%w,y=Math.floor(q/w);if(x===0||y===0||x===w-1||y===h-1)border=true;for(const n of [x? q-1:-1,x<w-1?q+1:-1,y?q-w:-1,y<h-1?q+w:-1])if(n>=0&&!seen[n]&&eligible(n)){seen[n]=1;queue[end++]=n;}}let dark=0,light=0;for(let k=0;k<end;k++){const v=d[queue[k]*4];if(v<220)dark++;if(v>240)light++;}const checker=end>25&&dark/end>.30&&light/end>.18;if(border||checker)for(let k=0;k<end;k++){const i=queue[k]*4;d[i]=d[i+1]=d[i+2]=d[i+3]=0;}}}
...
const {data:d}=await sharp(file).extract({left,top,width:w,height:h}).ensureAlpha().raw().toBuffer({resolveWithObject:true});if(!m.hasAlpha)neutralAlpha(d,w,h,solo||kind==='intro'?9:5);if(kind==='intro')cleanFragments(d,w,h);else keepCouple(d,w,h);
