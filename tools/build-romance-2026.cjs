...
for(const [name,keys]of [['couple-hand-kiss',['kiss-a','kiss-b']],['couple-dance',['dance-a','dance-b','dance-c','dance-d']]]){
 ...
 for(const key of keys){const {data,info}=await sharp(sources[key]).ensureAlpha().raw().toBuffer({resolveWithObject:true});
  ...
  for(let j=0;j<4;j++){const left=Math.floor(j%2*info.width/2),top=Math.floor(Math.floor(j/2)*info.height/2),width=Math.floor((j%2+1)*info.width/2)-left,height=Math.floor((Math.floor(j/2)+1)*info.height/2)-top;
   ...
   frames.push({x:dx,y:dy,w:cell,h:cell,anchorX:dx+px+(minX+maxX)/2,anchorY:dy+cell-29,referenceHeight:name==='couple-hand-kiss'?maxY-minY+1:height*.95});
  }
 }
 const file=name==='couple-hand-kiss'?'hand-kiss-new-8.webp':'dance-new-16.webp';
