...
function blocked(){return $('gift-treasure')?.open||$('journey-help').open||$('love-question').open||manualPause||document.hidden||$('letter').open||$('journal').open||$('challenge').open;}
function setPhase(p){
...
function tickPickup(){
 ...
 const oldX=x;x=mix(p.from,p.to,ease(clock/.35));gait+=Math.abs(x-oldX)/(meta['girl-walk']?.strideDistance||150);
...
function drawFlowerIntro(now){
 ...
 if(phase==='flowerAccept')sprite('intro-giving',Math.min(7,Math.floor((clock-.5)/4.3*8)),center,BASE,meta['girl-walk']?.displayHeight||245);
 else{const fade=ease(clock/1.3);sprite('intro-giving',7,center,BASE,meta['girl-walk']?.displayHeight||245,false,0,1-fade);ctx.save();ctx.globalAlpha=fade;drawGirl(x-camera,false,now);ctx.restore();}
}
...
function tick(dt){if(blocked())return;const previousBoy=cinematic.boy,previousPhase=phase;clock+=dt;total+=dt;wind=Math.max(0,wind-dt*.38);
 if(phase.startsWith('flower')){tickFlowerIntro(dt);}
  ...
  x=clamp(x+velocity*dt,START,limit);if(input)face=input;if(velocity)gait+=dt*Math.abs(velocity)/(meta['girl-walk']?.strideDistance||150);
 ...
 else if(phase==='approach'){const duration=Math.max(.85,Math.abs(cinematic.boy-148-cinematic.startGirl)/115),old=cinematic.girl;cinematic.girl=mix(cinematic.startGirl,cinematic.boy-148,stroll(clock/duration));gait+=Math.abs(cinematic.girl-old)/(meta['girl-walk']?.strideDistance||150);if(clock>=duration+.1){setPhase('offer');$('live').textContent='He offers his hand.';}}
 else if(phase==='hold'){if(clock>=3.2){setPhase('handKiss');$('live').textContent='He gently lifts her hand and kisses it.'}}
 else if(phase==='handKiss'){if(clock>=6.4)setPhase('dance');}
...
function drawGirl(px,walking,now){
 ...
 const sheet=usePickup?pickupSheet:hasBouquet?(moving?'girl-bouquet-walk':'girl-bouquet-pickup'):moving?'girl-walk':'girl-pickup';
 ...
 const handover=picking&&pickup.target==='bouquet'&&pickup.taken?ease((clock-(meta['girl-pickup']?.duration||2.8)*.55)/.18):1;
 ...
 if(picking){const p=pickup,index=p.target==='bouquet'?0:2,groundY=BASE-(index===0?36:25);
  if(!p.taken)prop(index,p.itemX-camera,groundY,index===0?90:55);
   ...
   const hx=px+(f.handX-f.anchorX)*scale*face,hy=BASE+(f.handY-f.anchorY)*scale,t=ease((clock-(meta['girl-pickup']?.duration||2.8)*.55)/.2);
...
function romancePhase(p=phase){return ['hold','handKiss','dance','embrace','loveQuestion','kiss','release'].includes(p);}
function danceSettings(){
 const frames=(meta['couple-dance']?.frames.length||8)+(meta['couple-dance-lean']?.frames.length||0);
...
function romancePose(){
 if(phase==='hold')return {sheet:'couple-hand-kiss',frame:0};
 ...
 if(phase==='dance'){const d=danceSettings(),n=reduce?0:Math.floor(clock*d.fps)%d.frames,spin=meta['couple-dance']?.frames.length||8;return n<spin?{sheet:'couple-dance',frame:n}:{sheet:'couple-dance-lean',frame:n-spin};}
 if(phase==='embrace')return {sheet:'couple-embrace',frame:[0,2,3][Math.min(2,Math.floor(clock/1.8*3))]};
 if(phase==='loveQuestion')return {sheet:'couple-embrace',frame:3};
 if(phase==='kiss')return {sheet:'couple-embrace',frame:clock<.5?3:clock<1.6?4:5};
 return {sheet:'couple-embrace',frame:clock<1.4?6:7};
}
function drawRomance(now){
 const center=(cinematic.girl+cinematic.boy)/2,{sheet,frame}=romancePose();
 const sway=reduce||phase==='handKiss'?0:phase==='dance'?Math.sin(clock*Math.PI/2)*2:Math.sin(clock*1.4)*.6;
 ...
 sprite(sheet,frame,center+sway,BASE,meta['girl-walk']?.displayHeight||245,false,0,alpha);
...
function startChallenge(i,overrideGate=null,onGift=null){stop();$('challenge').classList.toggle('gift-encounter',!!onGift);$('challenge').classList.toggle('kiss-sequence',(overrideGate||letters[i].gate)?.type==='kiss-sequence');const gate=overrideGate||letters[i].gate;mini={i,onGift,type:gate.type,t:0,taps:0,target:Math.floor(Math.random()*(Math.max(0,(gate.maxTaps||12)-(gate.minTaps||8))+1))+(gate.minTaps||8),caught:new Set(),step:0,matched:new Set(),flipped:[],flipAge:0,won:false,winAge:0,shake:0,particles:[]};$('challenge-number').textContent=`ADVENTURE ${i+1} / ${letters.length}`;$('challenge-title').textContent=gate.title||'A little secret';$('challenge-hint').textContent=gate.hint||'';$('challenge-status').textContent='';$('challenge').querySelectorAll('.keepsake-decor,.party-burst').forEach(el=>el.remove());$('mini-buttons').replaceChildren();$('playfield').hidden=gate.type==='question';$('question-form').hidden=gate.type!=='question';miniSize();
 ...
 if(gate.type==='kiss-sequence'){
 const state=mini;state.busy=false;
 const pickOrder=gate.choices.map((choice,j)=>({choice,j}));
 for(let n=pickOrder.length-1;n>0;n--){const swap=Math.floor(Math.random()*(n+1));[pickOrder[n],pickOrder[swap]]=[pickOrder[swap],pickOrder[n]];}
 // Keep the displayed list from accidentally matching the answer sequence.
 if(pickOrder.length>1&&pickOrder.every((item,n)=>item.j===n))[pickOrder[0],pickOrder[1]]=[pickOrder[1],pickOrder[0]];
 pickOrder.forEach(({choice,j})=>{const b=make('kiss-choice',choice,async()=>{
  if(mini!==state||state.won||state.busy||b.disabled)return;
  if(j!==state.step){$('challenge-status').textContent=gate.wrongMessage;b.classList.remove('kiss-wrong');void b.offsetWidth;b.classList.add('kiss-wrong');return;}
  state.busy=true;b.disabled=true;state.step++;b.classList.add('kiss-chosen');$('challenge-status').textContent=state.step+' / '+gate.choices.length+' kisses ♡';
  for(let n=0;n<12;n++){const star=document.createElement('i');star.textContent='✦';star.setAttribute('aria-hidden','true');star.style.setProperty('--dx',Math.cos(n*Math.PI/6)*55+'px');star.style.setProperty('--dy',Math.sin(n*Math.PI/6)*45+'px');b.append(star);}
  const audio=new Audio(letters[i].folder+'/'+(gate.sound||'kiss.wav'));state.kissAudio=audio;audio.volume=.45;audio.muted=muted;
  const soundDone=new Promise(resolve=>{let finished=false;const done=()=>{if(finished)return;finished=true;clearTimeout(timeout);audio.pause();resolve();};const timeout=setTimeout(done,15000);audio.onended=done;audio.onerror=done;audio.play()?.catch(done);state.cancelKiss=done;});
  await Promise.all([soundDone,new Promise(resolve=>setTimeout(resolve,600))]);
  if(mini!==state||!$('challenge').open)return;b.hidden=true;state.busy=false;state.kissAudio=null;
  if(state.step===gate.choices.length)winChallenge();else $('mini-buttons').querySelector('.kiss-choice:not([disabled])')?.focus({preventScroll:true});
 });b.textContent=choice;});
 }
 else if(gate.type==='bloom'){
mini.growth=0;mini.watering=false;const b=make('water-flower','Hold to water the flower',()=>{});b.textContent='Hold to water 💧';const begin=()=>{if(!mini||mini.won)return;mini.watering=true;};const end=()=>{if(mini)mini.watering=false;};b.onpointerdown=e=>{b.setPointerCapture(e.pointerId);begin();};b.onpointerup=end;b.onpointercancel=end;b.onlostpointercapture=end;b.onblur=end;b.onkeydown=e=>{if(e.key===' '||e.key==='Enter'){e.preventDefault();begin();}};b.onkeyup=e=>{if(e.key===' '||e.key==='Enter'){e.preventDefault();end();}};}
 else if(gate.type==='treasure'){make('chest-hit','Tap the treasure chest',()=>{if(!mini||mini.won)return;mini.taps++;mini.shake=1;miniBurst(200,185);chime('tap');$('challenge-status').textContent=`${mini.taps} / ${mini.target}`;if(mini.taps>=mini.target)winChallenge();});}
...
function miniTick(dt){if(!mini||!$('challenge').open||manualPause||document.hidden)return;mini.t+=dt;mini.shake=Math.max(0,mini.shake-dt*4);if(mini.won){mini.winAge+=dt;if(mini.winAge>1.05){const i=mini.i,onGift=mini.onGift;$('challenge').close();if(onGift)onGift();else openLetter(i);return;}}if(mini.type==='memory'&&mini.flipped.length===2){mini.flipAge+=dt;if(mini.flipAge>.95){for(const j of mini.flipped){const b=$('mini-buttons').children[j];b.classList.remove('flipped');b.textContent='✧';}mini.flipped=[];}}
 ...
 if(mini.type==='bloom'){mc.textAlign='center';mc.font=`${65+mini.growth*22}px Georgia`;mc.fillText(mini.growth<.8?'🌱':mini.growth<1.7?'🌷':'🌸',200,190);mc.font='23px Georgia';if(mini.watering)mc.fillText('💧',200,65);mc.fillStyle='#ffffff25';mc.fillRect(95,230,210,8);mc.fillStyle='#ffc4dd';mc.fillRect(95,230,210*mini.growth/2.4,8);mc.textAlign='start';}
 if(mini.type==='treasure'){miniProp(mini.won?3:2,200+Math.sin(mini.t*65)*mini.shake*6,190-(mini.onGift&&!mini.won&&!reduce?Math.abs(Math.sin(mini.t*2))*7:0),230,Math.sin(mini.t*65)*mini.shake*.03);if(mini.taps&&!mini.won){mc.strokeStyle='#ffe5b9';mc.lineWidth=2;for(let j=0;j<Math.min(mini.taps,6);j++){mc.beginPath();mc.moveTo(155+j*18,150);mc.lineTo(166+j*15,171);mc.lineTo(157+j*18,195);mc.stroke();}}}
...
function frame(ms){
 ...
 if(!manualPause){tick(drawDt);miniTick(drawDt);}ctx.clearRect(0,0,V,1000);drawWorld(total);updateLetterThanks(manualPause?0:drawDt);window.updateSkyLove?.(drawDt,active&&!blocked()&&!['entrance','video','finished','cutCake'].includes(phase));effects(manualPause?0:drawDt,total);
}
requestAnimationFrame(frame);
