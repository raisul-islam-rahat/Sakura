...
window.startGiftTreasure=(number,onGift)=>{startChallenge(number-1,{type:'treasure',title:'A treasure, just for you ♡',hint:'Tap the treasure chest to discover your gift.',minTaps:2,maxTaps:13},onGift);$('challenge-number').textContent='YOUR BIRTHDAY GIFT';};
...
function winChallenge(){if(!mini||mini.won)return;mini.won=true;mini.winAge=0;if(!mini.onGift)passed.add(mini.i);$('challenge-status').textContent=mini.onGift?'Your gift is waiting inside! ♡':'This little letter is yours. ♡';chime('win');miniBurst(200,180);}
...
$('close-challenge').onclick=()=>$('challenge').close();$('challenge').addEventListener('close',()=>{if($('challenge').open)return;mini=null;updateAction();});
