...
function openLetter(i){stop();modalLetter={...letters[i],i};const l=modalLetter;collected.add(i);passed.add(i);$('letter-number').textContent=`${String(i+1).padStart(2,'0')} / ${letters.length} · FOR ${C.nickname.toUpperCase()}`;$('letter-title').textContent=l.title;$('message').textContent=l.message.replaceAll('{{nickname}}',C.nickname).replaceAll('{{name}}',C.recipientName).replaceAll('{{sender}}',C.senderName);$('signature').textContent=l.signature||`With love, ${C.senderName}`;$('photo').hidden=!l.picture;if(l.picture){$('photo-img').src=l.picture;$('photo-img').alt=l.imageAlt||'Our little memory';}$('caption').textContent=l.caption||'';$('letter-animation').hidden=!l.animation.file;if(l.animation.file)$('letter-animation').src=`${l.folder}/${l.animation.file}`;$('continue').textContent=i===letters.length-1?'A little further… ♡':'Keep it in my heart ♡';$('letter').showModal();$('letter').scrollTop=0;window.showLetterGift?.(i+1,l.challengeEnabled!==false&&passed.has(i));window.writeBirthdayLetter?.($('message').textContent,i);if(l.music){song.src=l.music;song.load();}soundSync();updateCount();updateAction();}
