...
(()=>{const bound=new WeakSet();function attach(){document.querySelectorAll('dialog').forEach(dialog=>{if(bound.has(dialog))return;bound.add(dialog);dialog.classList.add('custom-scroll');const rail=document.createElement('div'),thumb=document.createElement('div');rail.className='card-scroll-rail';thumb.className='card-scroll-thumb';rail.setAttribute('aria-hidden','true');rail.append(thumb);dialog.prepend(rail);let pending=false;function update(){pending=false;const range=dialog.scrollHeight-dialog.clientHeight;rail.hidden=range<2;thumb.style.transform='translateY('+(12+Math.max(0,dialog.scrollTop)/Math.max(1,range)*Math.max(0,dialog.clientHeight-64))+'px)';}function queue(){if(!pending){pending=true;requestAnimationFrame(update);}}dialog.addEventListener('scroll',queue,{passive:true});new ResizeObserver(queue).observe(dialog);new MutationObserver(queue).observe(dialog,{attributes:true,attributeFilter:['open'],childList:true,subtree:true});let start=0,scroll=0;thumb.onpointerdown=e=>{start=e.clientY;scroll=dialog.scrollTop;thumb.setPointerCapture(e.pointerId);e.preventDefault();};thumb.onpointermove=e=>{if(thumb.hasPointerCapture(e.pointerId))dialog.scrollTop=scroll+(e.clientY-start)*(dialog.scrollHeight-dialog.clientHeight)/Math.max(1,dialog.clientHeight-64);};queue();});}new MutationObserver(attach).observe(document.body,{childList:true,subtree:true});attach();})();
