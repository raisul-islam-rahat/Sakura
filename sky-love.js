(()=>{
 ...
 window.updateSkyLove=(dt,visible)=>{
  ...
  if(!showing){wait-=dt;if(wait>0)return;const text=nextLine();let charIndex=0;el.replaceChildren();text.split(' ').forEach((word,index)=>{if(index)el.append(' ');const wordEl=document.createElement('span');wordEl.className='sky-word';Array.from(word).forEach(char=>{const span=document.createElement('span');span.className='sky-letter';if(charIndex++%4===0)span.classList.add('star-tip');span.textContent=char;span.dataset.char=char;span.style.setProperty('--glow-delay',(-Math.random()*6)+'s');span.style.setProperty('--glow-duration',(2.8+Math.random()*3.2)+'s');wordEl.append(span);});el.append(wordEl);});needsFit=true;paintAge=0;age=0;showing=true;el.style.top=(13+Math.random()*8)+'%';el.style.setProperty('--sky-x',(35+Math.random()*30)+'%');}
