(() => {
 ...
 window.writeBirthdayLetter=(text,index)=>{
  ...
  window.decorateKeepsake?.(paper,index);box.scrollLeft=box.scrollTop=0;box.replaceChildren();const accessible=document.createElement('span');accessible.className='sr-only';accessible.textContent=text;
  ...
  finish=()=>{cancelAnimationFrame(raf);written.textContent=text;remaining.textContent='';pen.hidden=true;};
