...
assert(visible>10000);frames.push(data);const input=await sharp(data,{raw:info}).flatten({background:'#17223a'}).resize(320,360).png().toBuffer(),i=tiles.length;tiles.push({input,left:i%4*320,top:Math.floor(i/4)*360});}
...
await sharp({create:{width:1280,height:2160,channels:4,background:'#17223a'}}).composite(tiles).png().toFile('output/continuous-romance-contact.png');
await sharp(Buffer.concat(frames),{raw:{width:640,height:720*24,channels:4,pageHeight:720}}).webp({lossless:true,loop:0,delay:[650,650,750,900,1100,900,700,750,...Array(16).fill(500)]}).toFile('output/continuous-romance-playback.webp');
